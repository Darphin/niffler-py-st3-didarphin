...
def test_add_category(envs, auth_token, clear_all_db_for_test_user):
    name = "new_cat"
...
def test_update_category(envs, auth_token, clear_all_db_for_test_user, category):
    name = "new_dog"
    client = SpendsHttpClient(envs.gateway_url, auth_token)
...
def test_delete_category(envs, auth_token, clear_all_db_for_test_user, category):
...
def test_delete_category_with_spend(envs, auth_token, clear_all_db_for_test_user, category):
...
def test_category_count_constraint(envs, auth_token, clear_all_db_for_test_user):
    spend_client = SpendsHttpClient(envs.gateway_url, auth_token)
