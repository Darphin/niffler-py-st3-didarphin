...
class TestSpendingUI:
    @Pages.auth_page
    def test_incorrect_spending(self, clear_all_db_for_test_user, envs):
        spend_page.open_page(envs.spending_url)
        spend_page.click_add()
        spend_page.check_amount_error()

    @Pages.auth_page
    def test_new_spending(self, clear_all_db_for_test_user, category, envs):
